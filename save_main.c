...
typedef struct {
    char *buffer;
    size_t buffer_length; //unsigned int, length of the buffer can't be negative
    ssize_t input_length;//signed int, on 32-bits machine,it's equal to int, otherwise it's equal to long in on 64-bits machine
} InputBuffer;
...
// macro to get the size of an attribute in a struct
...
typedef struct Cursor {
    ...
    uint32_t page_num;
    uint32_t cell_num;
    bool end_of_table; //indicates a position one past the last row
} Cursor;
...
// NodeType already declared at top via forward declarations
...
void print_constans(){
    ...
    printf("LEAF_NODE_CELLS_SIZE: %d\n", LEAF_NODE_CELL_SIZE);
    printf("LEAF_NODE_SPACE_FOR_CELLS: %d\n", LEAF_NODE_SPACE_FOR_CELLS);
    printf("LEAD_NODE_MAX_CELLS: %d\n", LEAF_NODE_MAX_CELLS);
}
...
void internal_node_insert(Table *table,uint32_t parent_page_num,uint32_t child_page_num){
    ...
    //An internal node with right child of INVALID_PAGE_NUM is empty
...
Table *dp_open(const char *filename){
...
/*
until we start recycling fre pages,new pages will always
...
// Return a pointer to an internal node cell (child+key pair)
void *internal_node_cell(void *node, uint32_t cell_num){
...
uint32_t *internal_node_child(void *node,uint32_t child_num){
    ...
    if (child_num > num_keys){
        ...
        // return internal_node_child_cell(node,child_num);
...
uint32_t *internal_node_key(void *node,uint32_t key_num){
    // return internal_node_child_cell(node,key_num)+INTERNAL_NODE_CHILD_SIZE;
    ...
    *internal_node_right_child(old_node)=INVALID_PAGE_NUM;
    ...
    for(int i = INTERNAL_NODE_MAX_CELLS - 1; i > INTERNAL_NODE_MAX_CELLS / 2; i--){
        ...
        *node_parent(cur)=new_page_num;
        ...
        // decrement the number of keys in old_node (fix pointer decrement bug)
...
void leaf_node_split_and_insert(Cursor *cursor,uint32_t key,Row *value){
    ...
    */
    ...
    //update cell count on the leaf nodes
...
PrepareResult prepare_insert(InputBuffer *input_buffer, Statement *statement) {
    ...
    (void)strtok; // silence unused warning for some compilers
    // skip the first token ("insert")
    (void) strtok(input_buffer->buffer, " ");
